...
describe('ArrondissementController', () => {
  ...
  beforeEach(() => {
    ...
    arrondissementController = new ArrondissementController(arrondissementService);
  });

  describe('findAll', () => {
    it('should return an array of arrondissements', async () => {
      const result = ['test'];
      jest.spyOn(arrondissementService, 'getAll').mockImplementation(async () => result);

      expect(await arrondissementController.getAllArrondissements()).toBe(result);
    });
  });
});